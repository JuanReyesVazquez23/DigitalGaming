  ...
  async function openOrders() {
    ...
    try {
      ...
      for (const o of orders) {
        ...
        card.querySelector(".order-head strong").textContent = `Pedido #${o.id.slice(0, 8)}`;
        card.querySelector(".order-date").textContent = date;
        card.querySelector(".order-items").textContent = lines;
        card.querySelector(".order-total strong").textContent = formatPrice(o.total);
